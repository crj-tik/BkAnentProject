...
- [ ] 4.3 使用 Graph 原生并行分支和统一聚合节点执行多个领域，明确 all-of/any-of 策略、分支失败处理、结果合并和聚合后的下一路由。
...
- [ ] 6.4 增加并发版本校验、审批唯一约束或等价幂等保护，覆盖相同审批在并发请求下只能推进一次 Graph。
...
- [ ] 9.1 增加状态 schema、条件路由、计划校验、技能匹配、并行聚合、审批决策和 Checkpointer 适配器的单元测试。
...
- [ ] 9.4 执行 `agent-service` 模块测试、Maven 依赖收敛检查、最小微服务启动和 MCP/A2A smoke test，并记录 Spring AI 版本对齐后的兼容问题。
- [ ] 9.5 更新 Supervisor Graph、审批状态、恢复接口、状态字段和最小启动方式的开发文档，明确后续 Spring AI Alibaba `2.0.0-M1.1` 评估必须在独立变更中进行。
